    ...
    def get_groups(self):
        ...
        for rewards, group in self.embedded_data.groupby (['reward']):
            ...
            groups.append ({
                ...
                'average state': (np.sum (states / np.linalg.norm (states, 2, axis = 1)[:, np.newaxis],
                                          axis = 0) / size).reshape ((1, -1)),  # s_x^-
                'average action': (np.sum (actions / np.linalg.norm (actions, 2, axis = 1)[:, np.newaxis],
                                           axis = 0) / size).reshape ((1, -1))  # a_x^-
            })
        return groups
